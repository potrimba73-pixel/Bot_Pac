...
function updatePreview() {
  // Welcome text - CORRIGIDO: markdown bold e mencao azul
  const msg = document.getElementById("w-message").value || "";
  let previewMsg = escapeHtml(msg)
    .replace(/{user}/g, '<span style="color:#5865F2;font-weight:500;">@user</span>')
    .replace(/{username}/g, "Username")
    .replace(/{userid}/g, "123456789")
    .replace(/{membercount}/g, "514")
    .replace(/{server}/g, "Portugal Alfa Community")
    .replace(/{rules}/g, "#regras");
  // Markdown bold: **texto** -> <strong>texto</strong>
  previewMsg = previewMsg.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
  document.getElementById("preview-text").innerHTML = previewMsg;
  ...
  // Leave preview - CORRIGIDO
  const lMsg = document.getElementById("l-message").value || "";
  let leavePreview = escapeHtml(lMsg)
    .replace(/{username}/g, "Username")
    .replace(/{userid}/g, "123456789")
    .replace(/{membercount}/g, "513")
    .replace(/{server}/g, "Portugal Alfa Community");
  leavePreview = leavePreview.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
  document.getElementById("preview-leave-text").innerHTML = leavePreview;
...
function resetDefaults() {
  if (!confirm("Tens a certeza? Isto vai apagar todas as configuracoes personalizadas!")) return;
  config = JSON.parse(JSON.stringify(DEFAULT_CONFIG));
  localStorage.setItem("pac_dashboard_config", JSON.stringify(config));
  fillForm();
  updatePreview();
  fillPlaceholders();
  updateExport();
  showToast("🔄 Configuracoes resetadas para os defaults!");
}
